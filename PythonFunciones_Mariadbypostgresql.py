...
def mostrar_jugadores_por_club(cursor):
    # Mostrar los clubes disponibles en la base de datos
    try:
        ...
        print("Error al obtener los clubes:", e)
        return
...
def eliminar_entrenador(cursor, db):
    ...
    # Verificar si existe un entrenador con el DNI proporcionado
    try:
        cursor.execute(f"SELECT Nombre FROM Entrenador WHERE DNI = '{dni}'")
        entrenador = cursor.fetchone()
        if not entrenador:
            print(f"No hay entrenador con el DNI '{dni}'.")
            return
    except Exception as e:
        print("Error al verificar el DNI del entrenador:", e)
        return
